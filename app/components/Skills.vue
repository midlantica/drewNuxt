...
<style scoped>
  ...
  body.anthro .skillsGrid {
    ...
    /* gap-px + background color = divider lines between cells only */
    @apply grid-cols-[repeat(4,1fr)] gap-x-px gap-y-0;
    /* background: #1a3a1a; */
    border: none;
    padding: 0;
    ...
    @media (min-width: 1026px) {
      @apply grid-cols-[repeat(12,1fr)];
    }

    /* Each skill cell — no border, transparent bg so grid bg shows as dividers */
    div {
      @apply relative;
      border: none;
    ...
    svg {
      ...
      /* filter: saturate(0.2) brightness(0.7) sepia(0.3) hue-rotate(80deg)
        drop-shadow(0 0 1px rgba(51, 255, 51, 0.3)); */
